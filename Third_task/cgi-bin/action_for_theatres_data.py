...
def add_data_to_bd(self, name, place):
    self.run("CREATE(theatre:Theatre)"
             "SET theatre.name = $name, theatre.place = $place", name=name, place=place)


driver = GraphDatabase.driver("neo4j://localhost:7687", auth=("neo4j", "admin"))
with driver.session() as session:
    session.write_transaction(add_data_to_bd, name_of_theatre, place_of_theatre)
driver.close()
